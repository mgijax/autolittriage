...
#DATERANGE = '2010/01/01:2016/12/31'
DATERANGE = '2013/11/01:2013/12/31' 	# 1 yr at a time for Plos 1 and Sci Rep

COMMON_SEARCH = [ "%s[DP] AND %s" % (DATERANGE, OPEN_ACCESS_CLAUSE) ]

# Journals and list of search parameters to match for articles.
#journalsToSearch = { \
#    "aging cell" : COMMON_SEARCH,	# open access, immediate
#    "plos biology"  : COMMON_SEARCH,	# ISSN "1544-9173"
#    "Biol Open" : COMMON_SEARCH,
#    "Dev Biol" : COMMON_SEARCH,
#}
# Add open access clause to each search param
#for j, paramList in journalsToSearch.items():
#    journalsToSearch[j] = [ "%s AND %s" % (p, OPEN_ACCESS_CLAUSE)
#						for p in paramList ]

def buildJournalSearch(files, journal=None):
    journalsToSearch = {}
    if journal != None:
	journalsToSearch[journal.strip()] = COMMON_SEARCH
    else:
	for file in files:
	    fp = open(file, 'r')
	    ...
	    for j in journals:
		journalsToSearch[j.strip()] = COMMON_SEARCH
...
def process():
    ...
    journalsToSearch = buildJournalSearch(args.inputFiles,
...
class PMCfileRangler (object):
	...
	"""
	self.dispatcher = Dispatcher.Dispatcher(maxProcesses=10)
	self.cmdIndexes = []
	self.cmds = []
	self.articles = []
