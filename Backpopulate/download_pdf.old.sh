...
if [ "${suffix}" == "gz" ]; then
	...
	pdfPath=`tar tvf ${directory}/${filename} | ./find_pdf_in_tar.py`

	if [ "${pdfPath}" != "" ]; then
		# This will extract pdfPath into cwd, not ${directory}.
		# I'd feel safer if this was extracted in ${directory}
		# Linux: tar -x ${pdfPath} -f ${directory}/${filename}
		# for MacOS tar:
		tar -xf ${directory}/${filename} ${pdfPath}
		exitIfFailed $? "failed to pull ${pdfPath} from gzip file"

		mv ${pdfPath} ${directory}/${finalPDFname}
